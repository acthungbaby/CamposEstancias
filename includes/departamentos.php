...
class Departamentos
{
	protected static $table_name = "departamentos";
	protected static $db_fields = array('departamento_id', 'departamento_nombre', 'region_id');
	...
	public function attach_file($file,$campo)
	{
		...
		if(move_uploaded_file($this->temp_path, $target_path))
		{
			...
			// Success
			$tn_image = new Thumbnail($this->upload_dir ."/". $this->foto, 280, 200, 0);
	...
	{
		$result_array = self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE departamento_id={$id} LIMIT 1");
	...
	public function save()
	{
		...
		return isset($this->departamento_id) ? $this->update() : $this->create();
	}
	...
	public function create()
	{
		...
		if($database->query($sql))
		{
			$this->departamento_id = $database->insert_id();
			return true;
		}
		else
		{
	...
	public function update()
	{
		...
		$sql .= " WHERE id='". $database->escape_value($this->departamento_id) ."'";
