...
<html class="no-js">
  <head>
	...
	function initialize() {
		...
		$sql = "SELECT * FROM propiedades WHERE habilitado = 1 ORDER BY id DESC ";
		$propiedades = Propiedades::find_by_sql($sql);
		
		?>
		<?php foreach($propiedades as $propiedad): ?>
		var point = new GLatLng(-(<?=$propiedad->latitud?>), -(<?=$propiedad->longitud?>));
		var breve = "<?php echo str_replace('\n','',$propiedad->breve); ?>";
		//var breve="Campos natural para aplicacion ganadera con aptitudes para transformacion a agricola (arroz), con un 80% de aprovechamiento.";
